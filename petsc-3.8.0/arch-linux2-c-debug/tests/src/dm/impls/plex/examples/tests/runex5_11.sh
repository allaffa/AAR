...
exec='../ex5'
testname='runex5_11'
label='dm_impls_plex_tests-ex5_11'
runfiles=''
...
petsc_testrun "${mpiexec} -n ${nsize} ${exec} ${args} " ex5_11.tmp ${testname}.err "${label}" 
res=$?

if test $res = 0; then
   petsc_testrun "${diff_exe} /sunhome/mlupopa/priv/PhD_project/Codes/Linear_Solver/AAR_MPI/petsc-3.8.0/src/dm/impls/plex/examples/tests/output/ex5_11.out ex5_11.tmp" diff-${testname}.out diff-${testname}.out diff-${label} ""
else
