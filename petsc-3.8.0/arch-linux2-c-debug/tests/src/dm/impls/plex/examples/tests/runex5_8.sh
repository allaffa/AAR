...
exec='../ex5'
testname='runex5_8'
label='dm_impls_plex_tests-ex5_8'
runfiles=''
...
petsc_testrun "${mpiexec} -n ${nsize} ${exec} ${args} " ex5_8.tmp ${testname}.err "${label}" 
res=$?

if test $res = 0; then
   petsc_testrun "${diff_exe} /sunhome/mlupopa/priv/PhD_project/Codes/Linear_Solver/AAR_MPI/petsc-3.8.0/src/dm/impls/plex/examples/tests/output/ex5_8.out ex5_8.tmp" diff-${testname}.out diff-${testname}.out diff-${label} ""
else
