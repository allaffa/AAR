...
exec='../ex1'
testname='runex1_refine_overlap_1'
label='dm_impls_plex_tests-ex1_refine_overlap_1'
runfiles=''
...
petsc_testrun "${mpiexec} -n ${nsize} ${exec} ${args} " ex1_refine_overlap_1.tmp ${testname}.err "${label}" 
res=$?

if test $res = 0; then
   petsc_testrun "${diff_exe} /sunhome/mlupopa/priv/PhD_project/Codes/Linear_Solver/AAR_MPI/petsc-3.8.0/src/dm/impls/plex/examples/tests/output/ex1_refine_overlap_1.out ex1_refine_overlap_1.tmp" diff-${testname}.out diff-${testname}.out diff-${label} ""
else
