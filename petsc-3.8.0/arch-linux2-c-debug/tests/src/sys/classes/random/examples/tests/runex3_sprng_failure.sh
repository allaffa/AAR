...
exec='../ex3'
testname='runex3_sprng_failure'
label='sys_classes_random_tests-ex3_sprng_failure'
runfiles=''
...
if ! $force; then
    printf "ok ${label} # SKIP PETSC_HAVE_SPRNG requirement not met\n"
    total=1; skip=1
...
petsc_testrun "${mpiexec} -n ${nsize} ${exec} ${args} " ex3_sprng_failure.tmp ${testname}.err "${label}" 
res=$?

if test $res = 0; then
   petsc_testrun "${diff_exe} /sunhome/mlupopa/priv/PhD_project/Codes/Linear_Solver/AAR_MPI/petsc-3.8.0/src/sys/classes/random/examples/tests/output/ex3_sprng_failure.out ex3_sprng_failure.tmp" diff-${testname}.out diff-${testname}.out diff-${label} ""
else
