...
exec='../ex3'
testname='runex3_rander48_failure'
label='sys_classes_random_tests-ex3_rander48_failure'
runfiles=''
...
petsc_testrun "${mpiexec} -n ${nsize} ${exec} ${args} " ex3_rander48_failure.tmp ${testname}.err "${label}" 
res=$?

if test $res = 0; then
   petsc_testrun "${diff_exe} /sunhome/mlupopa/priv/PhD_project/Codes/Linear_Solver/AAR_MPI/petsc-3.8.0/src/sys/classes/random/examples/tests/output/ex3_rander48_failure.out ex3_rander48_failure.tmp" diff-${testname}.out diff-${testname}.out diff-${label} ""
else
