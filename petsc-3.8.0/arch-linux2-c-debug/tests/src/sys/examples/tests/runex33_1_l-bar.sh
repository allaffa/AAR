...
exec='../ex33'
testname='runex33_1_l-bar'
label='sys_tests-ex33_1_l-bar'
runfiles=''
...
for nsize in 1,2; do
   for c in d "e,f" g; do
      for h in "i,j" "k"; do
         ...
         petsc_testrun "echo ${args}" ex33_1_l-bar.tmp ${testname}.err cmd-${label}_nsize-${nsize}_c-${c}_h-${h} 
         res=$?

         if test $res = 0; then
            petsc_testrun "${diff_exe} /sunhome/mlupopa/priv/PhD_project/Codes/Linear_Solver/AAR_MPI/petsc-3.8.0/src/sys/examples/tests/output/ex33_1_l-bar.out ex33_1_l-bar.tmp" diff-${testname}.out diff-${testname}.out diff-${label}_nsize-${nsize}_c-${c}_h-${h} ""
         else
