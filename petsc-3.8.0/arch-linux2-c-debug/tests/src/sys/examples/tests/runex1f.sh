...
petsc_testrun "${mpiexec} -n ${nsize} ${exec} ${args} " ex1f_1.tmp ${testname}.err "${label}" 'grep  "My error handler" | sort -b'
res=$?

if test $res = 0; then
   petsc_testrun "sort -b /sunhome/mlupopa/priv/PhD_project/Codes/Linear_Solver/AAR_MPI/petsc-3.8.0/src/sys/examples/tests/output/ex1f_1.out | ${diff_exe} - ex1f_1.tmp" diff-${testname}.out diff-${testname}.out diff-${label} ""
else
