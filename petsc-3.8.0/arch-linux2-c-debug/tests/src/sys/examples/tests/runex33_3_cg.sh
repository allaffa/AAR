...
exec='../ex33'
testname='runex33_3_cg'
label='sys_tests-ex33_3_cg'
runfiles=''
...
for c in d "e,f" g; do
   ...
   petsc_testrun "echo ${args}" ex33_3_cg.tmp ${testname}.err cmd-${label}_c-${c} 
   res=$?

   if test $res = 0; then
      petsc_testrun "${diff_exe} /sunhome/mlupopa/priv/PhD_project/Codes/Linear_Solver/AAR_MPI/petsc-3.8.0/src/sys/examples/tests/output/ex33_3_cg.out ex33_3_cg.tmp" diff-${testname}.out diff-${testname}.out diff-${label}_c-${c} ""
   else
