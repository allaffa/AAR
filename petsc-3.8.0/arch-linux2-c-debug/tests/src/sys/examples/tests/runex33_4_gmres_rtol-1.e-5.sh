...
exec='../ex33'
testname='runex33_4_gmres_rtol-1.e-5'
label='sys_tests-ex33_4_gmres_rtol-1.e-5'
runfiles=''
...
for c in d "e,f" g; do
   ...
   petsc_testrun "echo ${args}" ex33_4_gmres_rtol-1.e-5.tmp ${testname}.err cmd-${label}_c-${c} 
   res=$?

   if test $res = 0; then
      petsc_testrun "${diff_exe} /sunhome/mlupopa/priv/PhD_project/Codes/Linear_Solver/AAR_MPI/petsc-3.8.0/src/sys/examples/tests/output/ex33_4_gmres_rtol-1.e-5.out ex33_4_gmres_rtol-1.e-5.tmp" diff-${testname}.out diff-${testname}.out diff-${label}_c-${c} ""
   else
