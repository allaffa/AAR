...
petsc_testrun "${mpiexec} -n ${nsize} ${exec} ${args} " ex4_1.tmp ${testname}.err "${label}" 'sort -b | grep -v saws_port_auto_selectcd'
res=$?

if test $res = 0; then
   petsc_testrun "sort -b /sunhome/mlupopa/priv/PhD_project/Codes/Linear_Solver/AAR_MPI/petsc-3.8.0/src/sys/examples/tutorials/output/ex4_1.out | ${diff_exe} - ex4_1.tmp" diff-${testname}.out diff-${testname}.out diff-${label} ""
else
