...
exec='../ex10f90'
testname='runex10f90_1'
label='sys_tutorials-ex10f90_1'
runfiles='petsc.yml'
...
if ! $force; then
    printf "ok ${label} # SKIP PETSC_HAVE_YAML requirement not met\n"
    total=1; skip=1
...
petsc_testrun "${mpiexec} -n ${nsize} ${exec} ${args} " ex10f90_1.tmp ${testname}.err "${label}" 'grep -v saws_port_auto_select |grep -v malloc_dump | grep -v display'
res=$?

if test $res = 0; then
   petsc_testrun "${diff_exe} /sunhome/mlupopa/priv/PhD_project/Codes/Linear_Solver/AAR_MPI/petsc-3.8.0/src/sys/examples/tutorials/output/ex10f90_1.out ex10f90_1.tmp" diff-${testname}.out diff-${testname}.out diff-${label} ""
else
