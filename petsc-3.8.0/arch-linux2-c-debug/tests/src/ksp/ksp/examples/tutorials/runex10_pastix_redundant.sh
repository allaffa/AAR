...
exec='../ex10'
testname='runex10_pastix_redundant'
label='ksp_ksp_tutorials-ex10_pastix_redundant'
runfiles=''
...
args='-f0 ${DATAFILESPATH}/matrices/medium -ksp_type preonly -pc_type redundant -redundant_pc_factor_mat_solver_package pastix -num_numfac 2 -num_rhs 2'
timeoutfactor=1
...
for pc_redundant_number in 8 7 6 5 4 3 2 1; do
   ...
   petsc_testrun "${mpiexec} -n ${nsize} ${exec} ${args}  -pc_redundant_number ${pc_redundant_number}" ex10_pastix_redundant.tmp ${testname}.err "${label}_pc_redundant_number-${pc_redundant_number}" 
   res=$?

   if test $res = 0; then
      petsc_testrun "${diff_exe} /sunhome/mlupopa/priv/PhD_project/Codes/Linear_Solver/AAR_MPI/petsc-3.8.0/src/ksp/ksp/examples/tutorials/output/ex10_mumps_redundant.out ex10_pastix_redundant.tmp" diff-${testname}.out diff-${testname}.out diff-${label}_pc_redundant_number-${pc_redundant_number} ""
   else
