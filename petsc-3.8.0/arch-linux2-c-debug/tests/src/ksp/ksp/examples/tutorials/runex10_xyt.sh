...
exec='../ex10'
testname='runex10_xyt'
label='ksp_ksp_tutorials-ex10_xyt'
runfiles=''
...
args='-ksp_monitor_short -ksp_view -f0 ${DATAFILESPATH}/matrices/arco1 -ksp_type gmres -pc_type tfs'
timeoutfactor=1
...
petsc_testrun "${mpiexec} -n ${nsize} ${exec} ${args} " ex10_xyt.tmp ${testname}.err "${label}" 
res=$?

if test $res = 0; then
   petsc_testrun "${diff_exe} /sunhome/mlupopa/priv/PhD_project/Codes/Linear_Solver/AAR_MPI/petsc-3.8.0/src/ksp/ksp/examples/tutorials/output/ex10_xyt.out ex10_xyt.tmp" diff-${testname}.out diff-${testname}.out diff-${label} ""
else
