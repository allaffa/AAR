...
exec='../ex10'
testname='runex10_12'
label='ksp_ksp_tutorials-ex10_12'
runfiles=''
...
if ! $force; then
    printf "ok ${label} # SKIP PETSC_HAVE_MATLAB requirement not met\n"
    total=1; skip=1
...
petsc_testrun "${mpiexec} -n ${nsize} ${exec} ${args} " ex10_12.tmp ${testname}.err "${label}" 
res=$?

if test $res = 0; then
   petsc_testrun "${diff_exe} /sunhome/mlupopa/priv/PhD_project/Codes/Linear_Solver/AAR_MPI/petsc-3.8.0/src/ksp/ksp/examples/tutorials/output/ex10_12.out ex10_12.tmp" diff-${testname}.out diff-${testname}.out diff-${label} ""
else
