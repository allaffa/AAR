...
exec='../ex10'
testname='runex10_11'
label='ksp_ksp_tutorials-ex10_11'
runfiles=''
...
if ! $force; then
    printf "ok ${label} # TODO Need to determine goal of this test\n"
    total=1; todo=1
...
petsc_testrun "${mpiexec} -n ${nsize} ${exec} ${args} " ex10_11.tmp ${testname}.err "${label}" 
res=$?

if test $res = 0; then
   petsc_testrun "${diff_exe} /sunhome/mlupopa/priv/PhD_project/Codes/Linear_Solver/AAR_MPI/petsc-3.8.0/src/ksp/ksp/examples/tutorials/output/ex10_11.out ex10_11.tmp" diff-${testname}.out diff-${testname}.out diff-${label} ""
else
