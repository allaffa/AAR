...
exec='../ex10'
testname='runex10_asm_viennacl'
label='ksp_ksp_tutorials-ex10_asm_viennacl'
runfiles=''
...
if ! $force; then
    printf "ok ${label} # TODO No output file. Need to determine if deprecated\n"
    total=1; todo=1
...
if ! $force; then
    printf "ok ${label} # SKIP PETSC_HAVE_VIENNACL requirement not met\n"
    total=1; skip=1
    petsc_testend "/sunhome/mlupopa/priv/PhD_project/Codes/Linear_Solver/AAR_MPI/petsc-3.8.0/arch-linux2-c-debug/tests" 
    exit
fi
...
petsc_testrun "${mpiexec} -n ${nsize} ${exec} ${args} " ex10_asm_viennacl.tmp ${testname}.err "${label}" 
res=$?

if test $res = 0; then
   petsc_testrun "${diff_exe} /sunhome/mlupopa/priv/PhD_project/Codes/Linear_Solver/AAR_MPI/petsc-3.8.0/src/ksp/ksp/examples/tutorials/output/ex10_asm_viennacl.out ex10_asm_viennacl.tmp" diff-${testname}.out diff-${testname}.out diff-${label} ""
else
