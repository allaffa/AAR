...
exec='../ex10'
testname='runex10_zeropivot'
label='ksp_ksp_tutorials-ex10_zeropivot'
runfiles=''
...
petsc_testrun "${mpiexec} -n ${nsize} ${exec} ${args}  -ksp_pc_type bjacobi" ex10_zeropivot.tmp ${testname}.err "${label}" 
res=$?

if test $res = 0; then
   petsc_testrun "${diff_exe} /sunhome/mlupopa/priv/PhD_project/Codes/Linear_Solver/AAR_MPI/petsc-3.8.0/src/ksp/ksp/examples/tutorials/output/ex10_zeropivot.out ex10_zeropivot.tmp" diff-${testname}.out diff-${testname}.out diff-${label} ""
else
   printf "ok ${label} # SKIP Command failed so no diff\n"
fi

petsc_testrun "${mpiexec} -n ${nsize} ${exec} ${args}  -ksp_ksp_type cg -ksp_pc_type bjacobi -ksp_pc_bjacobi_blocks 1" ex10_zeropivot.tmp ${testname}.err "${label}" 
res=$?
