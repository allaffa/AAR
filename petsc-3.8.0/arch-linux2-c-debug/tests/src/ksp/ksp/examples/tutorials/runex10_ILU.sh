...
exec='../ex10'
testname='runex10_ILU'
label='ksp_ksp_tutorials-ex10_ILU'
runfiles=''
...
petsc_testrun "${mpiexec} -n ${nsize} ${exec} ${args}  -f0 ${DATAFILESPATH}/matrices/small -pc_factor_levels 1" ex10_ILU.tmp ${testname}.err "${label}" 
res=$?

if test $res = 0; then
   petsc_testrun "${diff_exe} /sunhome/mlupopa/priv/PhD_project/Codes/Linear_Solver/AAR_MPI/petsc-3.8.0/src/ksp/ksp/examples/tutorials/output/ex10_ILU.out ex10_ILU.tmp" diff-${testname}.out diff-${testname}.out diff-${label} ""
else
   printf "ok ${label} # SKIP Command failed so no diff\n"
fi

petsc_testrun "${mpiexec} -n ${nsize} ${exec} ${args}  -mat_type baij" ex10_ILU.tmp ${testname}.err "${label}" 
res=$?
