...
exec='../ex10'
testname='runex10_9_matload_block_size-1_f'
label='ksp_ksp_tutorials-ex10_9_matload_block_size-1_f'
runfiles=''
...
petsc_testrun "${mpiexec} -n ${nsize} ${exec} ${args} " ex10_9_matload_block_size-1_f.tmp ${testname}.err "${label}" 
res=$?

if test $res = 0; then
   petsc_testrun "${diff_exe} /sunhome/mlupopa/priv/PhD_project/Codes/Linear_Solver/AAR_MPI/petsc-3.8.0/src/ksp/ksp/examples/tutorials/output/ex10_9_matload_block_size-1_f.out ex10_9_matload_block_size-1_f.tmp" diff-${testname}.out diff-${testname}.out diff-${label} ""
else
