...
exec='../ex10'
testname='runex10_3'
label='ksp_ksp_tutorials-ex10_3'
runfiles=''
...
args='-f0 ${DATAFILESPATH}/matrices/medium -ksp_type bicg -pc_type asm'
timeoutfactor=1
...
petsc_testrun "${mpiexec} -n ${nsize} ${exec} ${args} " ex10_3.tmp ${testname}.err "${label}" 
res=$?

if test $res = 0; then
   petsc_testrun "${diff_exe} /sunhome/mlupopa/priv/PhD_project/Codes/Linear_Solver/AAR_MPI/petsc-3.8.0/src/ksp/ksp/examples/tutorials/output/ex10_3.out ex10_3.tmp" diff-${testname}.out diff-${testname}.out diff-${label} ""
else
