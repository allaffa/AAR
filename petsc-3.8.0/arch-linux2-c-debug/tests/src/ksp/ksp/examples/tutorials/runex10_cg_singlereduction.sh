...
exec='../ex10'
testname='runex10_cg_singlereduction'
label='ksp_ksp_tutorials-ex10_cg_singlereduction'
runfiles=''
...
petsc_testrun "${mpiexec} -n ${nsize} ${exec} ${args}  -f0 ${DATAFILESPATH}/matrices/small -mat_type mpisbaij -ksp_type cg -pc_type eisenstat -ksp_monitor_short -ksp_converged_reason" ex10_cg_singlereduction.tmp ${testname}.err "${label}" 
res=$?

if test $res = 0; then
   petsc_testrun "${diff_exe} /sunhome/mlupopa/priv/PhD_project/Codes/Linear_Solver/AAR_MPI/petsc-3.8.0/src/ksp/ksp/examples/tutorials/output/ex10_cg_singlereduction.out ex10_cg_singlereduction.tmp" diff-${testname}.out diff-${testname}.out diff-${label} ""
else
   printf "ok ${label} # SKIP Command failed so no diff\n"
fi

petsc_testrun "${mpiexec} -n ${nsize} ${exec} ${args}  -ksp_cg_single_reduction" ex10_cg_singlereduction.tmp ${testname}.err "${label}" 
res=$?
