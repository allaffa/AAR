...
exec='../ex12'
testname='runex12_field_bc_p1_2'
label='snes_tutorials-ex12_field_bc_p1_2'
runfiles=''
...
petsc_testrun "${mpiexec} -n ${nsize} ${exec} ${args} " ex12_field_bc_p1_2.tmp ${testname}.err "${label}" 
res=$?

if test $res = 0; then
   petsc_testrun "${diff_exe} /sunhome/mlupopa/priv/PhD_project/Codes/Linear_Solver/AAR_MPI/petsc-3.8.0/src/snes/examples/tutorials/output/ex12_field_bc_p1_2.out ex12_field_bc_p1_2.tmp" diff-${testname}.out diff-${testname}.out diff-${label} ""
else
