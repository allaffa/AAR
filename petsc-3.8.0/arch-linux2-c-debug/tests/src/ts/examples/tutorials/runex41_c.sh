...
petsc_testrun "${mpiexec} -n ${nsize} ${exec} ${args} " ex41_c.tmp ${testname}.err "${label}" 'sort -b'
res=$?

if test $res = 0; then
   petsc_testrun "sort -b /sunhome/mlupopa/priv/PhD_project/Codes/Linear_Solver/AAR_MPI/petsc-3.8.0/src/ts/examples/tutorials/output/ex41_c.out | ${diff_exe} - ex41_c.tmp" diff-${testname}.out diff-${testname}.out diff-${label} ""
else
