...
petsc_testrun "${mpiexec} -n ${nsize} ${exec} ${args} " ex41_d.tmp ${testname}.err "${label}" 'sort -b'
res=$?

if test $res = 0; then
   petsc_testrun "sort -b /sunhome/mlupopa/priv/PhD_project/Codes/Linear_Solver/AAR_MPI/petsc-3.8.0/src/ts/examples/tutorials/output/ex41_d.out | ${diff_exe} - ex41_d.tmp" diff-${testname}.out diff-${testname}.out diff-${label} ""
else
