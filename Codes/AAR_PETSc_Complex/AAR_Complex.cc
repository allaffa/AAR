...
int main( int argc, char **argv )
{
  ...
  ComputeMatrixA(&aar);     // Matrix, A = paar->helmholtzOpr
  // NOTE: For a different problem, other than Helmholtz equation, provide the matrix through the variable "paar->helmholtzOpr" and right hand side through "paar->RHS". 
  ...
  PetscPrintf(PETSC_COMM_WORLD,"*************************************************************************** \n \n");
  // -------------- AAR solver --------------------------
  AAR(&aar); // AAR with preconditioning PC=0={Jacobi}, PC=1={ILU(0) block-Jacobi}     
  PetscPrintf(PETSC_COMM_WORLD,"*************************************************************************** \n \n");
...
void AAR(AAR_OBJ* pAAR)
{
  ...
  // solving Ax=b, helmholtzOpr=A, rhs=b, D=diag(A)=diag(-1/4pi*lap + Q*I)

  // Need to get the diagonal block of A
  Mat Dblock; // sequntial matrix contained in current processor
  MatGetDiagonalBlock(pAAR->helmholtzOpr,&Dblock);
  ...
  if(pAAR->pc==1)
    {
      PCSetType(prec,PCILU); //ILU(0), Block-Jacobi
      PetscPrintf(PETSC_COMM_WORLD,"AAR preconditioned with Block-Jacobi using ILU(0).\n");
    }
  else if(pAAR->pc==0)
    {
  ...
  lapack_complex_double *lapack_FtF,*lapack_Ftf; // structures to pass complex valued arrays to lapacke_zgelsd
  PetscMalloc(sizeof(lapack_complex_double)*(m_aar*m_aar),&lapack_FtF);
  PetscMalloc(sizeof(lapack_complex_double)*(m_aar),&lapack_Ftf);
  int info;
  ...
  while(relres>aar_tol && iter<=max_iter)
    {
      ...
      MatMult(pAAR->helmholtzOpr,phi_old_global,Matvec_global); // Matvec_global=A*phi_k  
      ...
      if(iter % p_aar == 0 && iter > 1)
	{
		  ...
		  for(jj=0;jj<lydim;jj++)
		    {
		      for(ii=0;ii<lxdim;ii++)	
             		{ 
			  temp_sum += (PetscConjComplex(DF[j][cnt]))*locarr2[cnt]; //DF'*phi_res
			  cnt+=1;
			}
		    }
		}
	  ...
	  for(k=0;k<lzdim;k++)
	      ...
	      for(j=0;j<lydim;j++)
		{
		  for(i=0;i<lxdim;i++)
		    {
		      temp_sum += PetscConjComplex(phi_res[k+zcor][j+ycor][i+xcor])*phi_res[k+zcor][j+ycor][i+xcor];
		    }
		}
	    }
	  FtF_temp[ctr_temp]=temp_sum;
 
          MPI_Allreduce(MPI_IN_PLACE,FtF_temp,2*(m_aar*m_aar+m_aar+1),MPI_DOUBLE,MPI_SUM,MPI_COMM_WORLD); // need factor 2 for length of array when it is complex

	  for(j=0;j<m_aar;j++)
	    {       
	      for(i=0;i<=j;i++)
		{     
                  ctr_temp=j*m_aar + i; // FtF(i,j) linear index is j*m_aar + i
                  lapack_FtF[j*m_aar + i].real = (double)PetscRealPart(FtF_temp[ctr_temp]);
                  lapack_FtF[j*m_aar + i].imag = (double)PetscImaginaryPart(FtF_temp[ctr_temp]);

                  lapack_FtF[i*m_aar + j].real = lapack_FtF[j*m_aar + i].real;
                  lapack_FtF[i*m_aar + j].imag = -lapack_FtF[j*m_aar + i].imag; // negative since comp conj         
		}
	    }
	  ...
	  for(j=0;j<m_aar;j++)
	    {       
              lapack_Ftf[ctr].real = (double)PetscRealPart(FtF_temp[ctr_temp]);
              lapack_Ftf[ctr].imag = (double)PetscImaginaryPart(FtF_temp[ctr_temp]);
	      ctr+=1;  
	      ctr_temp+=1;           
	    }

	  relres=(double)PetscRealPart(FtF_temp[ctr_temp]);
	  relres = sqrt(relres);
	  relres = relres/rhs_norm; // relative residual

	  //PetscPrintf(PETSC_COMM_WORLD,"Iteration=%d, Relres = %.14f \n",iter,relres);
      
	  info=LAPACKE_zgelsd(LAPACK_COL_MAJOR,m_aar,m_aar,1,lapack_FtF,m_aar,lapack_Ftf,m_aar,svec,-1.0,&lprank); // lapack_Ftf gets replaced by Yk
	  ...
	  for(j=0;j<m_aar;j++)
	    {       
	      Ftf[ctr]=lapack_Ftf[ctr].real + PETSC_i*lapack_Ftf[ctr].imag; //Yk weights
	      ctr+=1;             
	    }
...
void Objects_Create(AAR_OBJ* pAAR)
{
  ...
  if(comm_size == 1 ) 
    {
      DMCreateMatrix(pAAR->da,&pAAR->helmholtzOpr);
      DMSetMatType(pAAR->da,MATSEQSBAIJ); // complex symmetric
    }
  else 
    {
      DMCreateMatrix(pAAR->da,&pAAR->helmholtzOpr);
      DMSetMatType(pAAR->da,MATMPISBAIJ); // complex symmetric
    }
...
void ComputeMatrixA(AAR_OBJ* pAAR)
{
  ...
  PetscScalar Dinv_factor;
  Dinv_factor = (pAAR->coeffs[0]) + Qr + PETSC_i*Qi; // (-1/4pi)(Lap)+Q = Diag term
  ...
  MatScale(pAAR->helmholtzOpr,0.0);
  ...
  for(k=zcor; k<zcor+lzdim; k++)
      ...
      for(i=xcor; i<xcor+lxdim; i++)
	{
	  ...
	  val[colidx++]=pAAR->coeffs[0] + Qr + PETSC_i*Qi  ;
	  for(l=1;l<=o;l++)
	    {
	  ...
	  MatSetValuesStencil(pAAR->helmholtzOpr,1,&row,6*o+1,col,val,ADD_VALUES);
	}
  MatAssemblyBegin(pAAR->helmholtzOpr, MAT_FINAL_ASSEMBLY);
  MatAssemblyEnd(pAAR->helmholtzOpr, MAT_FINAL_ASSEMBLY); 
}
