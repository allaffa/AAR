...
typedef struct
{
  ...
  PetscInt aar_ne_compute;
  PetscInt aar_lsqr_compute;
  char prec_type[20];
  char permutation[20];
...
int main( int argc, char **argv )
{
  ...
  Reordering(&A, aar_parameters.permutation);
  ...
  if(aar_parameters.aar_lsqr_compute == 1)
    AAR_LSQR_SetUp(&aar_lsqr_solver, &A, &x_guess, &prec, 6, 10, aar_parameters.omega, aar_parameters.maxit, aar_parameters.solver_tol);

  //Set up the AAR_NE solver
  AAR_NE_SOLVER aar_ne_solver;
  if(aar_parameters.aar_ne_compute == 1)
    AAR_NE_SetUp(&aar_ne_solver, &A, &x_guess, &prec, 6, 10, aar_parameters.omega, aar_parameters.maxit, aar_parameters.solver_tol);
  ...
  do
    {
      ...
      {
        fscanf(fConfFile,"%s",&params->permutation);
      ...
      else if(strcmp(str, "aar_ne:")==0)
      {
        fscanf(fConfFile,"%d",&params->aar_ne_compute);    
      }
      else if(strcmp(str, "aar_lsqr:")==0)
      {
        fscanf(fConfFile,"%d",&params->aar_lsqr_compute);    
      }
...
void Reordering(Mat* A, char* permutation)
{

  MatOrderingType rtype;
  IS              isrow,iscol;            /* row and column permutations */

  if(strcmp(permutation, "rcm")==0)
  {
    PetscPrintf(PETSC_COMM_WORLD, " ------------------------- \n Reverse Cuthill-McKee REORDERING \n ---------------------------------------");
    rtype = MATORDERINGRCM;
  }
  else if(strcmp(permutation, "md")==0)
  {
    PetscPrintf(PETSC_COMM_WORLD, " ------------------------- \n Minimum Degree REORDERING \n ---------------------------------------");
    rtype = MATORDERINGQMD;
  }
  else if(strcmp(permutation, "nd")==0)
  {
  ...
  else
  {
    ...
    rtype = MATORDERINGNATURAL;
  }

  MatGetOrdering(*A,rtype,&isrow,&iscol);
