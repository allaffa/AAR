function [x, iter, reshist, SOL_HIST] = AAJ2_restart(A,b,x_guess,tol,max_iter,M1,M2,omega,beta,m,p, restart)
...
clear AndersonMixing_AAJ_restart
relres = tol+1;
count = 1;
...
%     relres = norm(M2\M1\(b-A*x_new))/norm(M2\M1\(b));
    reshist(count) = relres;
%     fprintf('Iteration: %d, Residual= %g \n',count,relres)
    x = x_new;    
    x_new = AndersonMixing_AAJ_restart(x_prev,  x_new-x_prev,  m,  count,  beta, p, restart);   
    x_prev = x_new;
    count = count + 1;
end
clear AndersonMixing_AAJ_restart
